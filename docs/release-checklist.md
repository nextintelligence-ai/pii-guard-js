...
- [ ] `/` 홈에서 단일 PDF 선택/드롭 → `/single` 이동 → 자동 탐지 → 적용 → 다운로드 확인
- [ ] `/` 홈에서 여러 PDF 선택/드롭 → `/batch` 이동 → 큐에 파일 추가 확인
- [ ] `/batch` 처리 시작 → 성공 job의 `성공 파일 저장` 및 개별 저장 확인
- [ ] post-check warning 파일 → `/batch/$jobId` 상세 검수 진입 → 보강 후 `다시 적용` 확인
